"""Adaptive alt-screen vs main-screen selection.

Ported from ``cc_clean.tui.screen_mode`` so every tool sharing this core layer
can make the same well-reasoned choice between:

* **Alt screen** (``\\033[?1049h`` / smcup): pristine full-screen takeover, but
  some terminal profiles (Apple Terminal, VS Code, certain tmux configs) refuse
  to restore previous content cleanly on exit, leaving the user looking at a
  blank screen.
* **Main screen**: write into the existing scrollback. Survives every terminal
  but the TUI must clear and home the cursor itself.
...
__all__ = [
    "ALT_ENTER_FALLBACK",
    "ALT_EXIT_FALLBACK",
    "KNOWN_ALT_TERM_PROGRAMS",
    "KNOWN_MAIN_TERM_PROGRAMS",
    "MAIN_ENTER_SEQUENCE",
    "MAIN_EXIT_SEQUENCE",
    "SCREEN_MODE_ENV_NAMES",
    "SCREEN_MODE_VALUES",
    "ScreenModeDecision",
    "TerminfoScreenCaps",
    "detect_terminfo_screen_caps",
    "normalize_screen_mode",
    "query_tmux_alternate_screen",
    "resolve_screen_mode",
]
